...
def test_funding_row_exact_line_ms_to_ns():
    ...
    assert funding_row_to_line(row, "BTCUSDT") == (
        "funding,symbol=BTCUSDT rate=-0.00003746,interval_hours=8i 1585699200005000000"
    )
    assert funding_row_to_line(row, "ETHUSDT").startswith("funding,symbol=ETHUSDT ")
...
def test_premium_row_exact_line_uses_open_time_and_ohlc():
    ...
    assert premium_row_to_line(row, "BTCUSDT") == (
...
def test_parse_cli_defaults_and_flags_in_any_position():
    assert parse_cli(["2024-03"]) == (["2024-03"], "BTCUSDT", False)
    assert parse_cli(["2024-02:2024-03", "--symbol", "ethusdt", "--funding-only"]) == (
        ["2024-02", "2024-03"], "ETHUSDT", True)
    # flags before the month spec must not shadow the --months branch check
    assert parse_cli(["--symbol", "ETHUSDT", "2024-05"]) == (["2024-05"], "ETHUSDT", False)


def test_parse_cli_no_months_returns_empty():
    assert parse_cli([])[0] == []
    assert parse_cli(["--funding-only"])[0] == []
